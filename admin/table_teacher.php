...
{
    $search = $_POST['search'];
    $sql = "SELECT * FROM `multiusercontrol`.`userteacher` WHERE CONCAT(`name`,`department`) LIKE '%".$search."%'";
    $result = filterTable($sql);
    
}
 else {
    $sql ="SELECT * FROM `multiusercontrol`.`userteacher`";
    $result = filterTable($sql);
}
...
<body>
    <h3>Details of All Teachers</h3>
<form action="table_teacher.php" method="post">
        <div class="search">
            <div class="col-md-4" style="margin-left: auto; margin-right: 0;">
                
                    <div class="input-group mb-3">
                        <input type="text" name="search" value="<?php if(isset($_GET['search'])){echo $_GET['search'];} ?>" class="form-control" placeholder="Search by name or department" >
